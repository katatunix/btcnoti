...
module Ethermine =
    ...
    let private parse id (data : JsonType.Root) =
        {   Id = id
            EffectiveHashRate = data.HashRate
            EthPerDay = (float data.EthPerMin) * 60.0 * 24.0
            Unpaid = (float data.Unpaid) / 1000000000000000000.0 }

    let getEthermineInfoRes proxyOp id =
    ...
    let getEthermineInfo proxyOp id =
        match getEthermineInfoRes proxyOp id with
        | Ok info -> Some info
        | Error _ -> None
